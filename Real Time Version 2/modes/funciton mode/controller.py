...
Max_speed = 100# max speed to avoid torque issues and overshot

Kp = 20# constant for p controller
kp = 0.7
kd = 0.3
#ip address 
...
def callback(data):
	...
	if ids is not None:
		...
		arucoParameters.minDistanceToBorder = 1
		arucoParameters.maxErroneousBitsInBorderRate = 0.3
		arucoParameters.errorCorrectionRate = 1
		arucoParameters.adaptiveThreshConstant = 10
		arucoParameters.minCornerDistanceRate = 0.02
		arucoParameters.cornerRefinementMaxIterations = 40
		arucoParameters.perspectiveRemovePixelPerCell = 12
...
def inverse_kinematics(Error,motion_msg):
	...
	motion_msg.x= (2*x/3-Error.theta*13)*Kp
	motion_msg.y= (-x/3+y/math.sqrt(3)-Error.theta*13)*Kp
	motion_msg.z = (-x/3-y/math.sqrt(3)-Error.theta*13)*Kp
